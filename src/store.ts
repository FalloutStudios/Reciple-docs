	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/reciple').then(toJSON, noop),
				fetch('https://api.github.com/repos/FalloutStudios/reciple').then(toJSON, noop),
				fetch('https://api.github.com/repos/FalloutStudios/reciple/stats/contributors').then(toJSON, noop),
			]);
